...
"""This is from https://github.com/RobinXL/inkml2img, thanks @RobinXL"""
...
def get_traces_data(inkml_file_abs_path):
    ...
    if traceGroupWrapper is not None:
        for traceGroup in traceGroupWrapper.findall(doc_namespace +
                                                    'traceGroup'):
...
def get_min_coords(trace_group):
    ...
    return min(min_x_coords), min(min_y_coords), max(max_x_coords), max(
        max_y_coords)
...
def interpolate(trace_group, trace_grp_height, trace_grp_width, box_size):
    ...
    for trace in trace_group:
        ...
        interpolated_trace = [[
            round(coord[0] * scale_factor),
            round(coord[1] * scale_factor)
        ] for coord in trace]
    ...
    for trace in trace_group:
        ...
        if len(trace) == 1:
            ...
            for pt_idx in range(len(trace) - 1):
                ...
                pattern_drawn[line(r0=int(trace[pt_idx][1]),
                                   c0=int(trace[pt_idx][0]),
                                   r1=int(trace[pt_idx + 1][1]),
                                   c1=int(trace[pt_idx + 1][0]))] = 0
